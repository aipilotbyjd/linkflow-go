...
-- ---------------------------------------------------------------------------
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_workflows_user_active 
    ON workflow.workflows(user_id, is_active) WHERE is_active = TRUE;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_workflows_updated_at 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_node_executions_execution_status 
    ON execution.node_executions(execution_id, status);

-- ---------------------------------------------------------------------------
-- Schedule Schema Indexes
-- ---------------------------------------------------------------------------
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_schedules_next_run 
    ON schedule.schedules(next_run_at) WHERE is_active = TRUE;

-- ---------------------------------------------------------------------------
-- Webhook Schema Indexes
-- ---------------------------------------------------------------------------
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_webhooks_path 
